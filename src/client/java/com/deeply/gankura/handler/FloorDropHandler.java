...
public class FloorDropHandler {
    ...
    public static void onFloorDropParticle(double x, double y, double z) {
        ...
        pending.add(BlockPos.ofFloored(x, y - 1, z));
    }
    ...
    private static void dismiss(BlockPos pos) {
        ...
        dismissed.put(pos.toImmutable(), System.currentTimeMillis());
    }
    ...
    private static void tick(MinecraftClient client) {
        if (!isActive() || client.world == null || client.player == null) {
    ...
    private static boolean isFloorDrop(MinecraftClient client, BlockPos pos) {
        if (client.world == null) return false;

        Box box = Box.of(Vec3d.ofCenter(pos), 1.0, 1.0, 1.0);
        int strings = 0;
        for (DisplayEntity.ItemDisplayEntity display :
                client.world.getEntitiesByClass(DisplayEntity.ItemDisplayEntity.class, box, e -> true)) {
