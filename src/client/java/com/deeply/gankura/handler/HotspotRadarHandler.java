...
public class HotspotRadarHandler {
    ...
    private static void tick(Minecraft client) {
        ...
        if (solved != null && solved.distanceTo(client.player.position()) < ARRIVE_DISTANCE) {
            solver.forgetSolved();
        }
    }

    private static boolean isHoldingRadar(Minecraft client) {
        ItemStack held = client.player.getMainHandItem();
        if (held.isEmpty()) return false;

        String name = held.getHoverName().getString().replaceAll("\u00a7[0-9a-fk-or]", "");
