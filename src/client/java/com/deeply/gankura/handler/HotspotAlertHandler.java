...
public class HotspotAlertHandler {
    ...
    private static Vec3 trackedPos;
    ...
    private static void tick(Minecraft client) {
        ...
        if (goneMillis != 0) {
            ...
            if (trackedPos != null && client.player.position().distanceTo(trackedPos) <= ALERT_RANGE) {
                alert(client);
            }
            reset();
            return;
        ...
        trackedPos = tracked == null ? null : tracked.position();
    }

    // 浮きのそばにある Hotspot のアーマースタンド。なければ null
    private static ArmorStand hotspotNear(Minecraft client, FishingHook bobber) {
        AABB box = bobber.getBoundingBox().inflate(HOOK_SEARCH_RADIUS);
    ...
    private static void alert(Minecraft client) {
        NotificationUtils.showTitle(client, Component.literal("§d§lHotspot §c§lis gone"), null,
