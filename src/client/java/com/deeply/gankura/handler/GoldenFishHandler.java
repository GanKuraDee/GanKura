...
public class GoldenFishHandler {
    ...
    private static void countRodThrow(MinecraftClient client) {
    ...
    // 数えが消える前に、投げ直すよう知らせる
    private static void warnRod(MinecraftClient client) {
    ...
    // 持ち物の中に溶岩用の竿があるか
    private static boolean findLavaRod(MinecraftClient client) {
        var inventory = client.player.getInventory();
        for (int i = 0; i < inventory.size(); i++) {
            if (isLavaRod(inventory.getStack(i))) return true;
        }
        return false;
    }
    ...
    private static void title(MinecraftClient client, String text) {
        NotificationUtils.showTitle(client, Text.literal(text), null, TITLE_FADE, TITLE_STAY, TITLE_FADE);
