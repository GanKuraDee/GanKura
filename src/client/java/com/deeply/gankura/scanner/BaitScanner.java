...
public class BaitScanner {
    ...
    private static void scan(MinecraftClient client) {
        ...
        for (Text line : lore.lines()) {
            ...
            apply(client, ScoreboardUtils.toLegacyString(stack.getName()).trim(), matcher.group("amount"));
            return;
        }
    ...
    private static void apply(MinecraftClient client, String bait, String amount) {
     ...
     */
    private static void checkLow(MinecraftClient client, String bait, int count) {
        ...
        NotificationUtils.showTitle(client,
                Text.literal("§c§lBait Low §e§l" + count), null,
                ALERT_TITLE_FADE, ALERT_TITLE_STAY, ALERT_TITLE_FADE);
        NotificationUtils.playSound(client, SoundEvents.ENTITY_EXPERIENCE_ORB_PICKUP, ALERT_SOUND_VOLUME, ALERT_SOUND_PITCH);
    }
