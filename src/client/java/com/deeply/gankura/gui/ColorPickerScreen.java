...
public class ColorPickerScreen extends Screen {
    ...
    private int rgb() {
        return MathHelper.hsvToRgb(hue, saturation, brightness) & 0xFFFFFF;
    }
    ...
    protected void init() {
        ...
        addDrawableChild(new AlphaSlider(fieldX, sliderY, contentWidth));
        ...
        hexBox = new TextFieldWidget(textRenderer, fieldX, hexY, hexWidth, WIDGET_HEIGHT, Text.literal("Color"));
        hexBox.setMaxLength(7);
        hexBox.setText(ColorPalette.toHex(rgb()));
        hexBox.setChangedListener(text -> {
            if (updatingHex) return;

            Integer color = ColorPalette.parse(text);
            if (color == null) {
                hexBox.setEditableColor(INVALID_COLOR);
            } else {
                hexBox.setEditableColor(TextFieldWidget.DEFAULT_EDITABLE_COLOR);
                setColor(color);
            }
        });
        ...
        addDrawableChild(ButtonWidget.builder(Text.literal("Done"), button -> {
            ...
            close();
        }).dimensions(centerX - buttonWidth - 4, buttonsY, buttonWidth, WIDGET_HEIGHT).build());

        addDrawableChild(ButtonWidget.builder(Text.literal("Cancel"), button -> close())
                .dimensions(centerX + 4, buttonsY, buttonWidth, WIDGET_HEIGHT).build());
    }
    ...
    public boolean mouseDragged(Click click, double dragX, double dragY) {
        if (draggingField) {
            pickFromField(click.x(), click.y());
            return true;
        }

        if (draggingHue) {
            pickFromHueBar(click.y());
            return true;
        }
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        drawHueBar(context);
        drawSwatches(context);
        drawPreview(context);

        context.drawText(textRenderer, Text.literal("Presets"), swatchesX, swatchesY - 11, LABEL_COLOR, false);
    }

    // 1ピクセル幅の縦グラデーションを横に並べる。左右が彩度、上下が明度
    private void drawSaturationValueField(DrawContext context) {
        for (int i = 0; i < fieldWidth; i++) {
            float columnSaturation = (float) i / (fieldWidth - 1);
            int top = 0xFF000000 | MathHelper.hsvToRgb(hue, columnSaturation, 1.0F);
            context.fillGradient(fieldX + i, fieldY, fieldX + i + 1, fieldY + fieldHeight, top, 0xFF000000);
        }

        outline(context, fieldX - 1, fieldY - 1, fieldWidth + 2, fieldHeight + 2, 0xFF000000);

        int markerX = fieldX + Math.round(saturation * (fieldWidth - 1));
        int markerY = fieldY + Math.round((1.0F - brightness) * (fieldHeight - 1));
        outline(context, markerX - 4, markerY - 4, 9, 9, MARKER_SHADOW);
        outline(context, markerX - 3, markerY - 3, 7, 7, MARKER_COLOR);
    }
    ...
    private void drawHueBar(DrawContext context) {
        ...
        for (int i = 0; i < segments; i++) {
            int top = 0xFF000000 | MathHelper.hsvToRgb((float) i / segments, 1.0F, 1.0F);
            int bottom = 0xFF000000 | MathHelper.hsvToRgb((float) (i + 1) / segments, 1.0F, 1.0F);
            int y0 = fieldY + fieldHeight * i / segments;
            int y1 = fieldY + fieldHeight * (i + 1) / segments;
            context.fillGradient(hueX, y0, hueX + HUE_WIDTH, y1, top, bottom);
        }

        outline(context, hueX - 1, fieldY - 1, HUE_WIDTH + 2, fieldHeight + 2, 0xFF000000);

        int markerY = fieldY + Math.round(hue * (fieldHeight - 1));
        context.fill(hueX - 2, markerY - 1, hueX + HUE_WIDTH + 2, markerY + 2, MARKER_SHADOW);
        context.fill(hueX - 1, markerY, hueX + HUE_WIDTH + 1, markerY + 1, MARKER_COLOR);
    }
    ...
    private void drawSwatches(DrawContext context) {
        ...
        for (int i = 0; i < colors.size(); i++) {
            ...
            context.fill(x, y, x + SWATCH_SIZE, y + SWATCH_SIZE, 0xFF000000 | colors.get(i));
            outline(context, x, y, SWATCH_SIZE, SWATCH_SIZE, 0xFF000000);
        }
    }
    ...
    private void drawPreview(DrawContext context) {
        ...
        context.fill(previewX, previewY, previewX + previewWidth / 2, bottom, 0xFFFFFFFF);
        context.fill(previewX + previewWidth / 2, previewY, right, bottom, 0xFF303030);
        context.fill(previewX, previewY, right, bottom, (fillAlpha << 24) | rgb());
        outline(context, previewX, previewY, previewWidth, WIDGET_HEIGHT, 0xFF000000 | rgb());
    }

    // DrawContext に枠線を引く口が無いので、4辺を細い矩形で塗る
    ...
    private class AlphaSlider extends SliderWidget {
        ...
        AlphaSlider(int x, int y, int width) {
            super(x, y, width, WIDGET_HEIGHT, Text.empty(), fillAlpha / 255.0D);
            updateMessage();
        }

        @Override
        protected void updateMessage() {
            setMessage(Text.literal("Fill opacity: " + Math.round(fillAlpha * 100.0F / 255.0F) + "%"));
        }
