...
public class GolemBeaconRenderer {
    ...
    public static void submitBeaconState(LevelRenderState worldState, Camera camera) {
        if (!ModConfig.Combat.TheEnd.showGolemWorldLocation_Beacon) return;
        ...
        state.blockEntityType = net.minecraft.world.level.block.entity.BlockEntityTypes.BEACON;
        ...
        Vec3 renderCenter = Vec3.atCenterOf(renderPos);
        double dx = camera.position().x - renderCenter.x;
        double dz = camera.position().z - renderCenter.z;
