...
public class EntityHighlightManager {
    ...
    private static void updateHighlights(MinecraftClient client) {
        ...
        if (goldenFishVisuals && GoldenFishHandler.isActive()) {
            for (Entity entity : client.world.getEntities()) {
                ...
                if (entity instanceof ArmorStandEntity stand && !stand.isMarker()) headOnlyGlowEntities.add(entity);
        ...
        if (isSafari && ModConfig.INSTANCE.foraging.enableSafariFishHighlight) {
            for (Entity entity : client.world.getEntities()) {
                if (!(entity instanceof FishEntity)) continue;
    ...
    private static void detectCrimsonNamedMobs(MinecraftClient client) {
    ...
    private static boolean detectNamedPlayerMobs(MinecraftClient client, List<MobVisual> candidates) {
        boolean found = false;
    ...
    private static boolean canDetectNamedPlayerMob(MinecraftClient client, MobVisual target) {
    ...
    private static MobVisual lotusAtollTarget(Entity entity) {
        if (entity instanceof TropicalFishEntity) return LotusAtoll.LOTUSFISH;
        if (entity instanceof TurtleEntity) return LotusAtoll.TEWTIL;
        if (entity instanceof DolphinEntity) return LotusAtoll.FLIPFLOPPER;
        if (entity instanceof GlowSquidEntity) return LotusAtoll.SEASHINE;

        if (entity instanceof FrogEntity frog) {
            // 大きいカエルは Sea Creature の Puddle Jumper / Frog Prince。
            // Frog Prince も緑だが、色を見る前にここで外れる
            if (frog.getWidth() > LOTUM_MAX_WIDTH) return null;
            // 残る 0.50 のカエルは Lotum(オレンジ)と Atoll Croaker(緑)の2択
            return frog.getVariant().matchesKey(FrogVariants.COLD) ? null : LotusAtoll.LOTUM;
        }
        return null;
    }
    ...
    private static MobVisual areaAnimalTarget(Entity entity) {
        ...
        if (entity instanceof RabbitEntity rabbit) {
            return rabbit.getVariant() == RabbitEntity.Variant.EVIL ? null : TorrhusCanyon.BUNBUN;
        }
    ...
    private static ShulkerBoxBlock shulkerBoxDisplayNear(MinecraftClient client, Entity entity) {
    ...
    private static ItemStack displayItem(DisplayEntity.ItemDisplayEntity display) {
        DisplayEntity.ItemDisplayEntity.Data state = display.getData();
    ...
    private static boolean hasNamedPlayerMob(MinecraftClient client, MobVisual target) {
        for (PlayerEntity player : client.world.getPlayers()) {
    ...
    private static List<Entity> connectedHeadStands(MinecraftClient client, Entity anchor, MobVisual target,
                                                    double searchRadius) {
        Box box = anchor.getBoundingBox().expand(searchRadius);
    ...
    private static String nameplateHealth(Entity visual, MobVisual target) {
        ...
        for (Entity nameTag : healthNameTags) {
            double distance = nameTag.squaredDistanceTo(visual);
            if (distance > bestDistance) continue;

            // 控えた後にネームタグが消えることもあるので、都度確かめる
            Text name = nameTag.getCustomName();
     ...
     */
    private static void announceSeaCreature(MinecraftClient client, MobVisual target, Entity entity) {
     ...
     */
    public static void showSeaCreatureCatchTitle(MinecraftClient client, MobVisual target, boolean doubleHook) {
    ...
    public static void showSeaCreatureTitle(MinecraftClient client, MobVisual target) {
        showSeaCreatureTitle(client, target, false, "");
    }

    private static void showSeaCreatureTitle(MinecraftClient client, MobVisual target, boolean fromCatch, String suffix) {
        ...
        client.execute(() -> {
            NotificationUtils.showTitle(client, Text.literal(text), null,
                    SEA_CREATURE_TITLE_FADE, SEA_CREATURE_TITLE_STAY, SEA_CREATURE_TITLE_FADE);
            NotificationUtils.playSound(client, SoundEvents.ENTITY_EXPERIENCE_ORB_PICKUP,
     ...
     */
    private static Entity seaCreatureVisual(MinecraftClient client, Entity nameTag, MobVisual target) {
        ...
        return headStandUnderNameTag(client.world.getEntitiesByClass(bodyType, box, filter), nameTag);
    }

    // ネームタグの下から、その Sea Creature のスキンを被ったヘッドを探す。
    // Water Hydra のようにモブ本体が被っていることもあるので、見るのは LivingEntity
    private static Entity skinnedHeadUnderNameTag(MinecraftClient client, Entity nameTag, MobVisual target) {
        Box box = nameTag.getBoundingBox().expand(NAMETAG_SEARCH_RADIUS);
        return headStandUnderNameTag(client.world.getEntitiesByClass(LivingEntity.class, box,
    ...
    private static boolean hasTypeTargetNear(MinecraftClient client, Entity nameTag, MobVisual target) {
        Box box = nameTag.getBoundingBox().expand(NAMETAG_SEARCH_RADIUS);
        for (Entity nearby : client.world.getEntitiesByClass(Entity.class, box, e -> true)) {
    ...
    private static MobVisual typeTargetOf(MinecraftClient client, Entity entity) {
        MobVisual target = areaAnimalTarget(entity);
    ...
    private static boolean hasSafariSkinNear(MinecraftClient client, Entity nameTag, MobVisual target) {
    ...
    private static Entity headStandWithSkinUnderNameTag(MinecraftClient client, Entity nameTag, String skinId) {
        Box box = nameTag.getBoundingBox().expand(NAMETAG_SEARCH_RADIUS);
        return headStandUnderNameTag(client.world.getEntitiesByClass(LivingEntity.class, box,
    ...
    private static Class<? extends Entity> seaCreatureBodyType(MobVisual target) {
        ...
        return switch (seaCreature) {
            case THE_LOCH_EMPEROR -> GuardianEntity.class;
            // Lotus Atoll のカエルは2種。乗り物の有無で見分ける(seaCreatureBodyMatches)
            case PUDDLE_JUMPER, FROG_PRINCE -> FrogEntity.class;
            case SILKBREEZE -> BreezeEntity.class;
            case THUNDER -> ElderGuardianEntity.class;
            case LORD_JAWBUS -> IronGolemEntity.class;
            case REINDRAKE -> EnderDragonEntity.class;
            case MITHRIL_GRUBBER, WATER_WORM, FLAMING_WORM -> SilverfishEntity.class;
            case OASIS_SHEEP -> SheepEntity.class;
            case OASIS_RABBIT, CARROT_KING -> RabbitEntity.class;
            case AGARIMOO -> MooshroomEntity.class;
            case POISONED_WATER_WORM -> EndermiteEntity.class;
            case LAVA_BLAZE -> BlazeEntity.class;
            case LAVA_PIGMAN -> ZombifiedPiglinEntity.class;
            case PLHLEGBLAST -> SquidEntity.class;
            default -> null;
        };
    }
    ...
    private static boolean seaCreatureBodyMatches(MobVisual target, Entity entity) {
        if (target == MobVisual.SeaCreature.CARROT_KING) {
            return entity instanceof RabbitEntity rabbit && rabbit.getVariant() == RabbitEntity.Variant.EVIL;
        }
        if (target == MobVisual.SeaCreature.OASIS_RABBIT) {
            return entity instanceof RabbitEntity rabbit && rabbit.getVariant() != RabbitEntity.Variant.EVIL;
        }
        // GlowSquid はイカの派生型なので、素のイカだけを本体とみなす
        if (target == MobVisual.SeaCreature.PLHLEGBLAST) return !(entity instanceof GlowSquidEntity);
    ...
    private static MobVisual seaCreatureTypeTarget(Entity entity) {
        ...
        if (GameState.Server.isCrimsonIsle()) {
            if (entity instanceof ElderGuardianEntity) return MobVisual.SeaCreature.THUNDER;
            if (entity instanceof IronGolemEntity) return MobVisual.SeaCreature.LORD_JAWBUS;
            return null;
        }
        if (GameState.Server.isTorrhusCanyon()) {
            return entity instanceof BreezeEntity ? MobVisual.SeaCreature.SILKBREEZE : null;
        }
        if (GameState.Server.isJerrysWorkshop()) {
            return entity instanceof EnderDragonEntity ? MobVisual.SeaCreature.REINDRAKE : null;
        }
        if (GameState.Server.isLotusAtoll()) {
            if (!(entity instanceof FrogEntity frog)) return null;
     ...
     */
    private static void applySeaCreatureVisuals(MinecraftClient client, MobVisual target, Entity body) {
        Entity visual = body;
     ...
     */
    private static void reportMissingHeadSkin(MinecraftClient client, Entity nameTag, MobVisual target) {
        reportMissingVisual(client, nameTag, target.plainLabel(), "known head skin");
    }

    // 実体名で見つかるはずのプレイヤーエンティティ型モブが、見つからないことを知らせる
    private static void reportMissingNamedPlayer(MinecraftClient client, Entity nameTag, MobVisual target) {
        reportMissingVisual(client, nameTag, target.plainLabel(), "named entity");
    }

    // ネームタグはあるのに、そのモブの本体が見つからないことを知らせる
    private static void reportMissingBody(MinecraftClient client, Entity nameTag, MobVisual target) {
        reportMissingVisual(client, nameTag, target.plainLabel(), "matching entity");
    }

    // 同じく本体が見つからないが、MobVisual ではないボス用
    private static void reportMissingBody(MinecraftClient client, Entity nameTag, String bossName) {
        reportMissingVisual(client, nameTag, bossName, "matching entity");
    }

    // 同じく実体名で探すが、MobVisual ではない Crimson Isle のボス用
    private static void reportMissingNamedPlayer(MinecraftClient client, Entity nameTag, String bossName) {
        reportMissingVisual(client, nameTag, bossName, "named entity");
    }

    private static void reportMissingVisual(MinecraftClient client, Entity nameTag, String name, String what) {
     ...
     */
    private static List<Entity> headBodyParts(MinecraftClient client, MobVisual target, Entity anyHead) {
    ...
    private static List<Entity> skinMatchedHeads(MinecraftClient client, Entity body, MobVisual target) {
        Box box = body.getBoundingBox().expand(HEAD_PARTS_SEARCH_RADIUS);

        List<Entity> parts = new ArrayList<>();
        for (ArmorStandEntity stand : client.world.getEntitiesByClass(ArmorStandEntity.class, box, stand -> true)) {
    ...
    private static List<String> headSkinValues(Entity entity) {
        if (!(entity instanceof ArmorStandEntity) && !(entity instanceof ZombieEntity)) return List.of();

        LivingEntity wearer = (LivingEntity) entity;
        if (wearer.getCustomName() != null) return List.of();

        return skinValues(wearer.getEquippedStack(EquipmentSlot.HEAD));
    }
    ...
    private static MobVisual safariDisplaySkinTarget(MinecraftClient client, Entity entity) {
        Box box = entity.getBoundingBox().expand(HITBOX_HEAD_RADIUS);
        for (DisplayEntity.ItemDisplayEntity display : client.world.getEntitiesByClass(DisplayEntity.ItemDisplayEntity.class, box, e -> true)) {
    ...
    private static Entity lochEmperorRider(Entity body) {
        for (Entity passenger : body.getPassengerList()) {
            if (passenger instanceof SkeletonEntity) return passenger;
        }
        return null;
    }
    ...
    private static boolean isLochEmperorBody(Entity entity) {
        if (!(entity instanceof GuardianEntity) || entity instanceof ElderGuardianEntity) return false;
        for (Entity passenger : entity.getPassengerList()) {
            if (passenger instanceof SkeletonEntity) return true;
        }
        return false;
    }
