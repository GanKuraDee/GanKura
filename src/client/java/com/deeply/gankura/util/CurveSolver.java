...
public class CurveSolver {
    ...
    private Vec3d solved;

    /** 軌跡を取り直す。レーダーを使った瞬間に呼ぶ */
    public void start(Vec3d eyePos) {
        points.clear();
        startPos = eyePos;
    }

    /** 直前の点から離れすぎていないか。別のパーティクルを拾わないため */
    public boolean isConnected(Vec3d pos) {
        Vec3d last = points.isEmpty() ? startPos : points.get(points.size() - 1);
    ...
    private Vec3d solve() {
        ...
        return new Vec3d(result[0], result[1] - 0.5, result[2]);
    }
